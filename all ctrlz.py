...
undo_stack = []  # Стек для хранения истории аннотаций
...
def clear_annotations():
    global current_annotations, label_dir, index, dataset, undo_stack
    undo_stack.append(list(shapes))  # Сохраняем текущее состояние перед очисткой
    current_annotations = []
    if index < len(dataset):
    ...
    if dragging_handle is not None:
        ...
        if current_shape is not None:
            ...
            if abs(end_x - start_x) > 0 and abs(end_y - start_y) > 0:
                undo_stack.append(list(shapes))  # Сохраняем текущее состояние перед добавлением нового бокса
...
def on_delete(event):
    global selected_boxes, shapes, undo_stack
    undo_stack.append(list(shapes))  # Сохраняем текущее состояние перед удалением
...
def change_class(event=None):
    ...
    def set_class():
        global current_class_id, selected_boxes, undo_stack
        new_class_id = class_combobox.current()
        if selected_boxes:
            undo_stack.append(list(shapes))  # Сохраняем текущее состояние перед изменением класса
